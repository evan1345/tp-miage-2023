...
*R/cette ligne de commande est utilisé pour demarrer une instance de contenneur Docker pour le systèlme de gestion de base de données PostgreSql
   ...
   Les options utilisées sont les suivantes :
    ...
    "-it": cCeci spécifie que le conteneur doit être exécuté en mode interactif et avec un pseudo-tty attaché. Dans le but pour l'utilisateur d'interagir avec le conteneur via le terminal.
    ...
    "-p 5432:5432": elle définit le port de communication entre le conteneur et le système hôte. Le port 5432 de l'hôte est mappé sur le port 5432 du conteneur.
